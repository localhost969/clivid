...
class AIVideoChatInterface:
    def __init__(self):
        ...
        # Multi-step outputs cache and limits
        self._step_outputs = {}
        self.max_multi_steps = 20
        # Auto-clean intermediate files produced during multi-step workflows
        self.auto_cleanup_intermediate = True
        
        # Available task modules
        self.available_tasks = {
            "list_videos": "List all video files in the directory",
            "list_audio": "List all audio files in the directory",
    ...
    def get_video_files(self):
        """Get list of available video files with broad container support and ffprobe fallback"""
        video_extensions = [
            '*.mp4', '*.m4v', '*.mov', '*.qt', '*.avi', '*.wmv', '*.asf', '*.mkv', '*.webm', '*.flv',
            '*.mpeg', '*.mpg', '*.mpe', '*.mpv', '*.m2v', '*.3gp', '*.3g2', '*.ogv', '*.vob',
            '*.ts', '*.m2ts', '*.mts', '*.mxf', '*.f4v', '*.dat', '*.dv', '*.dvr-ms', '*.rm', '*.rmvb', '*.ogm'
        ]
        candidates = []
        for extension in video_extensions:
            candidates.extend(glob.glob(extension))
            # Don't add uppercase versions as they're already covered by glob
        unique_files = set(candidates)
        # Fallback: probe for video streams in any file
        try:
            for path in glob.glob('*.*'):
                if path in unique_files:
                    continue
                if self._ffprobe_has_stream(path, 'v'):
    ...
    def get_audio_files(self):
        """Get list of available audio files with broad codec/container support and ffprobe fallback"""
        audio_extensions = [
            '*.mp3', '*.wav', '*.aac', '*.flac', '*.ogg', '*.oga', '*.m4a', '*.wma', '*.aiff', '*.aif', '*.aifc',
            '*.amr', '*.ape', '*.ac3', '*.eac3', '*.dts', '*.opus', '*.spx', '*.caf', '*.au', '*.snd', '*.mka'
        ]
        candidates = []
        for extension in audio_extensions:
            ...
            # Don't add uppercase versions as they're already covered by glob
        unique_files = set(candidates)
        # Fallback: probe for audio streams in any file
        try:
            for path in glob.glob('*.*'):
                if path in unique_files:
                    continue
                if self._ffprobe_has_stream(path, 'a'):
                    unique_files.add(path)
        except Exception:
            pass
        return sorted(list(unique_files))

    def _ffprobe_has_stream(self, file_path, stream_type):
        """Use ffprobe to check if file has a given stream type ('v' for video, 'a' for audio)."""
        try:
            cmd = [
                'ffprobe', '-v', 'error',
    ...
    def call_mistral_api(self, user_message, video_files_list):
        ...
        # Get audio files as well
        audio_files_list = self.get_audio_files()
        
        video_files_str = ", ".join(video_files_list) if video_files_list else "No video files found"
        audio_files_str = ", ".join(audio_files_list) if audio_files_list else "No audio files found"
        context = self.get_context_for_ai()
        
        system_prompt = f"""You are an AI assistant that helps with video and audio file operations. 

Available video files: {video_files_str}
Available audio files: {audio_files_str}

Available operations:
- list_videos: List all video files in directory (no parameters needed)
- list_audio: List all audio files in directory (no parameters needed)
...
Additionally for multi-step workflows, references may be:
- "stepN_output" (e.g., step1_output, step2_output) to refer to output from a specific step
- "prev_output" or "last_output" to refer to the most recent step output
- "last_audio" or "last_video" to refer to the most recent created audio/video
...
- "extract 0:30-1:00 from a.mp4 then replace audio in b.mp4 with it" → step1: extract_audio_segment; step2: replace_audio with audio_file="step1_output"
...
- Common patterns:
 ...
 - Each step should have complete parameters. Prefer cross-step references using "stepN_output" or "prev_output".
 - Support up to 20 steps in a single workflow when the user chains many actions"""
    ...
    def execute_task_with_params(self, intent, parameters):
        ...
        elif intent == "list_audio":
            return self.execute_list_audio()
    ...
    def execute_list_audio(self):
        """Execute list audio files task"""
        try:
            # Get audio files and display them
            ...
            print(f"[*] I found {len(audio_files)} audio file(s) in your directory:")
            for i, audio_file in enumerate(audio_files, 1):
                file_size = self.get_file_size(audio_file)
                print(f"  {i}. {audio_file} ({file_size})")
            
            return "Listed all audio files"
        except Exception as e:
            print(f"[!] Error: {e}")
            return f"Error: {e}"
    ...
    def execute_trim(self, video_file, start_time, end_time):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Record step output if in multi-step context
                if hasattr(self, '_current_step_num') and isinstance(getattr(self, '_current_step_num'), int):
                    self.update_step_output(self._current_step_num, output_file)
    ...
    def execute_replace_audio(self, video_file, audio_file, audio_handling=None, audio_codec=None):
        ...
        # Get available audio files (broad support)
        audio_files = self.get_audio_files()
    ...
    def show_help(self):
        ...
        print("  'Work with audio.mp3'")
        print("  'Convert audio.wav to mp3'")
        print("")
        ...
        print("  'Work with that audio file'")
        print("")
        print("  === Commands ===")
    ...
    def run(self):
        """Main chat loop"""
        self.clear_screen()
        print("CLIVID - CLI Video & Audio Assistant v1.0")
        print("=" * 50)
        print("Hi! I can help you work with video and audio files using natural language.")
        ...
        # Automatically show available video and audio files
        video_files = self.get_video_files()
        audio_files = self.get_audio_files()
        
        if video_files or audio_files:
            print(f"\n[*] I found {len(video_files)} video file(s) and {len(audio_files)} audio file(s) in your directory:")
            ...
            if audio_files:
                ...
                for i, audio_file in enumerate(audio_files, 1):
                    try:
                        ...
                        print(f"    {i}. {audio_file} ({size:.1f}MB)")
                    except:
                        print(f"    {i}. {audio_file}")
            
            print(f"\nJust tell me what you'd like to do with these files!")
        else:
            print("\n[!] No video or audio files found in the current directory.")
            print("Please add some video files (e.g., .mp4, .m4v, .mov, .avi, .mkv, .webm, .wmv, .flv, etc.)")
            print("or audio files (e.g., .mp3, .wav, .aac, .flac, .ogg, .m4a, etc.) and restart.")
        ...
        while True:
            try:
                ...
                # Get available video and audio files
                video_files = self.get_video_files()
                audio_files = self.get_audio_files()
    ...
    def execute_multi_step_operations(self, ai_result):
        """Execute multi-step workflows by delegating to tasks.multi_step"""
        multi_steps = ai_result.get("multi_step_operations", [])
        if not multi_steps:
            return "No multi-step operations found"
        try:
            multi_step_module = self._import_task_module('multi_step')
            if not multi_step_module:
                return "Error: Could not import multi_step module"
            return multi_step_module.run_multi_step(self, multi_steps)
        except Exception as e:
    ...
    def resolve_file_reference(self, file_reference, file_type=None):
        ...
        # Step-aware references (e.g., step1_output, prev_output)
        try:
            m = re.match(r'^step(\d+)(?:_output)?$', ref_lower)
            if m:
                step_num = int(m.group(1))
                if hasattr(self, '_step_outputs') and step_num in self._step_outputs:
                    return self._step_outputs[step_num]
        except Exception:
            pass
        if ref_lower in ['prev_output', 'previous_output', 'last_output', 'previous']:
            if hasattr(self, '_step_outputs') and self._step_outputs:
                try:
                    latest_step = max(self._step_outputs.keys())
                    return self._step_outputs[latest_step]
                except Exception:
                    pass
    ...
    def update_step_output(self, step_num, output_path):
        """Record the primary output path for a given step number in multi-step workflows."""
        try:
            self._step_outputs[int(step_num)] = output_path
        except Exception:
            pass
